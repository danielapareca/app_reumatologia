...
import { fasesDaDoenca, etapaSugerida, guiaFase, type ConsultaTipo } from '@/lib/clinical/faseGuia';
...
function SaveBtn({ label }: { label: string }) {
  ...
  return (
    <button type="submit" className="btn-primary" disabled={pending} style={{ maxWidth: 300 }}>
...
export default function NovoPaciente() {
  ...
  // ---- Passo 1: doença + tipo + fase ----
  const [doencaId, setDoencaId] = useState('');
  const [tipo, setTipo] = useState<ConsultaTipo>('primeira');
  const [etapa, setEtapa] = useState('');
  const fases = useMemo(() => fasesDaDoenca(doencaId), [doencaId]);
  const guia = useMemo(() => guiaFase(doencaId, etapa, tipo), [doencaId, etapa, tipo]);

  function onDoenca(v: string) {
    setDoencaId(v);
    setEtapa(etapaSugerida(v, tipo));
  }
  function onTipo(v: ConsultaTipo) {
    setTipo(v);
    if (doencaId) setEtapa(etapaSugerida(doencaId, v));
  }

  // ---- Passo 2: dados + documentos ----
  const [f, setF] = useState<Campos>(vazio);
  const upd = (k: keyof Campos, v: string) => setF((p) => ({ ...p, [k]: v }));
  function onNascimento(v: string) {
    setF((p) => { const calc = idadeFromNascimento(v); return { ...p, nascimento: v, idade: calc || p.idade }; });
  }
  ...
  async function lerDados(file: File) {
    ...
    try {
      ...
      const resp = await fetch('/api/extract-paciente', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ imageBase64: b64, mediaType: file.type }) });
      ...
      setF((p) => ({
        ...
        endereco: d.endereco || p.endereco, cidade: d.cidade || p.cidade, estado: d.estado || p.estado, cep: d.cep || p.cep,
      }));
    ...
    } finally { setLendoDados(false); }
  }

  // leitura de EXAMES (PDF/foto), múltiplos
  ...
  async function lerExames(files: File[]) {
    ...
    for (const file of files) {
      ...
      try {
        ...
        const resp = await fetch('/api/extract-exames', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
        const j = await resp.json().catch(() => ({}));
        ...
        const vals: ExamePend[] = (j.valores || []).map((v: { marcador?: string; valor?: number; unidade?: string; data?: string }) => ({
          marcador: String(v.marcador || ''), valor: String(v.valor ?? ''), unidade: String(v.unidade || ''), data: v.data || today, incluir: true,
        }));
      ...
      } finally { setLendoEx(false); }
    }
  }

  // ---- após cadastrar: salva exames/arquivos e abre a ficha já na fase escolhida ----
        ...
        try {
          ...
          if (user) {
            for (const { file, nome } of arquivos) {
              ...
              if (!up.error) await supabase.from('exam_files').insert({ patient_id: pid, doctor_id: user.id, path, filename: nome, data: today });
            }
          }
        } catch { /* ignora */ }
      }
      const qs = new URLSearchParams();
      if (doencaId) qs.set('doenca', doencaId);
      if (etapa) qs.set('etapa', etapa);
      qs.set('tipo', tipo);
      router.push(`/paciente/${pid}?${qs.toString()}`);
    })();
  }, [state.id, exames, arquivos, router, today, doencaId, etapa, tipo]);

  const incluidos = exames.filter((e) => e.incluir).length;
  const doencaNome = doencaId ? D[doencaId]?.n : '';
    ...
    <>
      ...
      <div className="page">
        ...
        <div className="wiz-steps">
          <div className={'wiz-step' + (passo === 1 ? ' on' : '') + (passo > 1 ? ' done' : '')}>
            <span className="wiz-n">{passo > 1 ? <Icon name="check" size={14} /> : '1'}</span> Doença e fase
          </div>
          <div className="wiz-conn" />
          <div className={'wiz-step' + (passo === 2 ? ' on' : '')}>
            <span className="wiz-n">2</span> Documentos e dados
          </div>
        </div>
        ...
        {passo === 1 && (
          <div style={{ maxWidth: 640 }}>
            <p className="psub">Comece pela <b>doença</b> e pela <b>fase</b>. O app usa isso para orientar o que preencher e já abrir a ficha na etapa certa.</p>

            <div className="field">
              <label>Doença / hipótese diagnóstica *</label>
              <select className="doenca" value={doencaId} onChange={(e) => onDoenca(e.target.value)}>
                <option value="">Selecione a doença…</option>
                {ORDER.map(([grupo, ids]) => (
                  <optgroup key={grupo} label={grupo}>
                    {ids.filter((id) => D[id]).map((id) => <option key={id} value={id}>{D[id].n}</option>)}
                  </optgroup>
                ))}
              </select>
            </div>
            ...
            <div className="field">
              ...
              <div className="chips">
                {(['primeira', 'retorno'] as const).map((v) => (
                  <span key={v} className={'chip' + (tipo === v ? ' on' : '')} onClick={() => onTipo(v)}>
            ...
            {doencaId && fases.length > 0 && (
              <div className="field">
                <label>Fase do tratamento</label>
                <div className="wiz-fases">
                  {fases.map((fa) => (
                    <label key={fa.id} className={'wiz-fase' + (etapa === fa.id ? ' on' : '')}>
                      <input type="radio" name="fase" checked={etapa === fa.id} onChange={() => setEtapa(fa.id)} />
                      <span>
                        <span className="wf-t">Fase {fa.ordem} · {fa.label}</span>
                        {fa.sub && <span className="wf-s">{fa.sub}</span>}
                      </span>
                    </label>
                  ))}
                </div>
            ...
            {/* orientação da fase */}
            <div className="fase-guia">
              <div className="fg-head"><Icon name="sparkles" size={16} /> O que preencher nesta fase{doencaNome ? ` — ${doencaNome}` : ''}</div>
              <ul>{guia.map((g, i) => <li key={i}>{g}</li>)}</ul>
            </div>

            <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
              <button className={'btn-primary' + (doencaId ? '' : ' disabled')} disabled={!doencaId} onClick={() => setPasso(2)} style={{ maxWidth: 260 }}>
                Continuar para os dados →
              </button>
            </div>
            <p className="wiz-skip">Sem doença definida ainda? <button type="button" className="linkbtn" onClick={() => setPasso(2)}>Pular e só cadastrar o paciente</button></p>
          </div>
        )}
        ...
        {passo === 2 && (
          <>
            {doencaId && (
              <div className="fase-resumo">
                <div>
                  <b>{doencaNome}</b> · {tipo === 'primeira' ? 'Primeira consulta' : 'Retorno'}{etapa && D[doencaId] ? ` · ${D[doencaId].etapas.find((e) => e.id === etapa)?.label || ''}` : ''}
                </div>
                <button type="button" className="linkbtn" onClick={() => setPasso(1)}>alterar</button>
              </div>
            )}

            <p className="psub">Mande o <b>print do sistema</b> e os <b>exames</b> (PDF ou foto) — a IA preenche — ou preencha manualmente. Tudo é revisado antes de cadastrar.</p>
              ...
              <div className="dc-grid">
                <div className="dc-col">
                  ...
                  <div className="dc-btns">
                    <label className={'btn-ghost' + (lendoDados ? ' disabled' : '')}>Anexar print
                      <input type="file" accept="image/*" disabled={lendoDados} style={{ display: 'none' }} onChange={(e) => { const file = e.target.files?.[0]; if (file) lerDados(file); e.target.value = ''; }} /></label>
                    <label className={'btn-ghost' + (lendoDados ? ' disabled' : '')}>Tirar foto
                      <input type="file" accept="image/*" capture="environment" disabled={lendoDados} style={{ display: 'none' }} onChange={(e) => { const file = e.target.files?.[0]; if (file) lerDados(file); e.target.value = ''; }} /></label>
                  </div>
                ...
                <div className="dc-col">
                  ...
                  <div className="dc-btns">
                    <label className={'btn-ghost' + (lendoEx ? ' disabled' : '')}>Anexar PDF/foto
                      <input type="file" accept="application/pdf,image/*" multiple disabled={lendoEx} style={{ display: 'none' }} onChange={(e) => { const fs = Array.from(e.target.files || []); if (fs.length) lerExames(fs); e.target.value = ''; }} /></label>
                    <label className={'btn-ghost' + (lendoEx ? ' disabled' : '')}>Tirar foto
                      <input type="file" accept="image/*" capture="environment" disabled={lendoEx} style={{ display: 'none' }} onChange={(e) => { const fs = Array.from(e.target.files || []); if (fs.length) lerExames(fs); e.target.value = ''; }} /></label>
                  </div>
            ...
            <form action={formAction} style={{ maxWidth: 560 }}>
              ...
              <div className="row2">
                <div className="field"><label>Data de nascimento</label>
                  <input name="nascimento" type="date" value={f.nascimento} onChange={(e) => onNascimento(e.target.value)} /></div>
                <div className="field"><label>Idade (preenchida pela data)</label>
                  <input name="idade" value={f.idade} onChange={(e) => upd('idade', e.target.value)} placeholder="ex.: 54 anos" /></div>
              </div>
              <div className="row2">
                <div className="field"><label>WhatsApp</label>
                  <input name="whats" placeholder="(11) 90000-0000" value={f.whats} onChange={(e) => upd('whats', e.target.value)} /></div>
                <div className="field"><label>CPF</label>
                  <input name="cpf" placeholder="000.000.000-00" value={f.cpf} onChange={(e) => upd('cpf', e.target.value)} /></div>
              </div>
              <div className="field"><label>E-mail</label>
                <input name="email" type="email" placeholder="email@exemplo.com" value={f.email} onChange={(e) => upd('email', e.target.value)} /></div>
              <div className="field"><label>Endereço (rua, número, bairro)</label>
                <input name="endereco" placeholder="Rua Exemplo, 123, Centro" value={f.endereco} onChange={(e) => upd('endereco', e.target.value)} /></div>
              <div className="row2">
                <div className="field"><label>Cidade</label>
                  <input name="cidade" placeholder="Cidade" value={f.cidade} onChange={(e) => upd('cidade', e.target.value)} /></div>
                <div className="field"><label>Estado (UF)</label>
                  <input name="estado" placeholder="SP" maxLength={2} value={f.estado} onChange={(e) => upd('estado', e.target.value)} /></div>
              </div>
              <div className="field" style={{ maxWidth: 200 }}><label>CEP</label>
                <input name="cep" placeholder="00000-000" value={f.cep} onChange={(e) => upd('cep', e.target.value)} /></div>
              ...
              <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
                <button type="button" className="btn-ghost" onClick={() => setPasso(1)} style={{ maxWidth: 120 }}>← Voltar</button>
                {finalizando
                  ? <button type="button" className="btn-primary" disabled style={{ maxWidth: 300 }}>Salvando e abrindo a ficha…</button>
