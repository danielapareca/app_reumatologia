...
export default function NavRail({ isAdmin }: { isAdmin: boolean }) {
  ...
  const items: { href: string; icon: string; label: string; on: boolean }[] = [
    ...
    ...(isAdmin ? [{ href: '/gestor', icon: 'dashboard', label: 'Painel do gestor', on: path.startsWith('/gestor') }] : []),
    { href: '/perfil', icon: 'user', label: 'Meu perfil', on: path.startsWith('/perfil') },
  ];
  ...
  return (
    <nav className="navrail no-print" aria-label="Navegação">
      ...
      <form action="/auth/signout" method="post" className="nr-out">
        <button type="submit" className="nr-link" title="Sair" aria-label="Sair"><Icon name="logout" size={20} /></button>
      </form>
    </nav>
  );
}
